...
export default async function analyzeComment({
  channel,
  comment,
  redFlags,
}: {
  ...
  const commentAuthor = {
    ...
    displayNameWithoutAccentsAndDiacritics: removeAccentsAndDiacritics(
  ...
  const handleContains = (input: string, contains: RedFlag['contains']) => {
    if (Array.isArray(contains)) {
      const successResults = contains
        .map((c) => handleContainsWithOptions(input, c))
        .filter((r) => r !== false)
  ...
  const handleContainsWithOptions = (
    input: string,
    contains: ContainsWithOptions,
  ): false | { additionalWeight: number } => {
    let additionalWeight = contains.weight || 0

    if ('similarity' in contains) {
      const { similarity: minSimilarity, value } = contains
      if (minSimilarity > stringSimilarity.compareTwoStrings(input, replaceTemplatesInValue(value)))
        return false
      return { additionalWeight }
    }
    if ('specialCharacters' in contains) {
      const { allowedCharacters } = contains
      if (!handleContainsRegExp(input, getSpecialCharacterRegExp({ allowedCharacters })))
        return false
      return { additionalWeight }
    } else if (Array.isArray(contains.value)) {
      const minNumberOfOccurrences =
        ('minNumberOfOccurrences' in contains ? contains.minNumberOfOccurrences : 1) ?? 1
      let numberOfOccurrences = 0
